...
class TrOCR(tlx.nn.Module):
    ...
    def loss_fn(self, logits, input_ids, attention_mask):
        ...
        labels = tlx.concat(
            [
                input_ids[:, 1:],
                tlx.ones([input_ids_shape[0], 1], dtype=input_ids.dtype),
            ],
            axis=1,
        )
        mask = tlx.concat(
            [
                attention_mask[:, 1:],
                tlx.zeros([input_ids_shape[0], 1], dtype=attention_mask.dtype),
            ],
            axis=1,
        )
    ...
    def forward(self, inputs, input_ids=None, attention_mask=None):
        ...
        encoder_hidden_states = outputs[0]
        logits = self.trocr_decoder(
            input_ids, attention_mask, encoder_hidden_states=encoder_hidden_states
        )
        return logits
