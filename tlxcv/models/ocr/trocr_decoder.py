...
def _make_causal_mask(input_ids_shape, dtype, past_key_values_length=0):
    ...
    if past_key_values_length > 0:
        mask = tlx.concat(
            [tlx.zeros((tgt_len, past_key_values_length), dtype=dtype), mask], axis=-1
        )
...
class TrOCRLearnedPositionalEmbedding(TrOCREmbedding):
    ...
    def __init__(self, num_embeddings: int, embedding_dim: int, **kwargs):
        ...
        self.offset = 2
        super().__init__(
            num_embeddings=num_embeddings + self.offset,
    ...
    def forward(self, input_shape, past_key_values_length: int = 0):
        ...
        positions = tlx.arange(
            past_key_values_length, seq_len + past_key_values_length, delta=1
        )
...
class TrOCRSinusoidalPositionalEmbedding(Module):
    ...
    def create_position_ids_from_input_ids(
        ...
        incremental_indices = (
            tlx.cumsum(mask, axis=1).type_as(mask) + past_key_values_length
        ) * mask
...
class TrOCRAttention(Module):
    ...
    def __init__(
        ...
        self.k_proj = tlx.nn.Linear(
            in_features=self.kdim,
            out_features=embed_dim,
            b_init="constant" if bias else None,
        )
        self.v_proj = tlx.nn.Linear(
            in_features=self.vdim,
            out_features=embed_dim,
            b_init="constant" if bias else None,
        )
        self.q_proj = tlx.nn.Linear(
            in_features=embed_dim,
            out_features=embed_dim,
            b_init="constant" if bias else None,
        )

        self.out_proj = tlx.nn.Linear(
            in_features=embed_dim,
            out_features=embed_dim,
            b_init="constant" if bias else None,
        )
    ...
    def _shape(self, tensor, seq_len: int, bsz: int):
        return tlx.transpose(
            tlx.reshape(tensor, [bsz, seq_len, self.num_heads, self.head_dim]),
            perm=[0, 2, 1, 3],
        )
    ...
    def forward(
        ...
        if attention_mask is not None:
            ...
            attn_weights = (
                tlx.reshape(attn_weights, (bsz, self.num_heads, tgt_len, src_len))
                + attention_mask
            )
            attn_weights = tlx.reshape(
    ...
    def __init__(
        ...
        if is_decoder:
            self.encoder_attn = TrOCRAttention(
                embed_dim=self.embed_dim,
                num_heads=decoder_attention_heads,
                kdim=cross_attention_hidden_size,
                vdim=cross_attention_hidden_size,
                dropout=attention_dropout,
                is_decoder=True,
        ...
        self.fc1 = tlx.nn.Linear(
            in_features=self.embed_dim, out_features=decoder_ffn_dim
        )
        self.fc2 = tlx.nn.Linear(
            in_features=decoder_ffn_dim, out_features=self.embed_dim
        )
...
class TrOCRDecoder(Module):
    def __init__(
        ...
        self.embed_scale = math.sqrt(hidden_size) if scale_embedding else 1.0

        self.embed_tokens = tlx.nn.Embedding(
        ...
        if use_learned_position_embeddings:
            ...
            self.embed_positions = TrOCRSinusoidalPositionalEmbedding(
                max_position_embeddings + self.padding_idx + 1,
                hidden_size,
                self.padding_idx,
            )

        if layernorm_embedding:
            self.layernorm_embedding = tlx.nn.LayerNorm(normalized_shape=hidden_size)
            self.layernorm_embedding.build([None, None, hidden_size])
        else:
            self.layernorm_embedding = None

        self.decode_layers = tlx.nn.ModuleList(
            [
                TrOCRDecoderLayer(
                    hidden_size,
                    decoder_attention_heads,
...
class TrOCRForCausalLM(Module):
    def __init__(
        ...
        self.output_projection = tlx.nn.Linear(
            in_features=hidden_size, out_features=vocab_size, b_init=None
        )
