...
class ConvNormActivation(nn.Sequential):
    ...
    def __init__(
        ...
        if norm_layer is not None:
            layers.append(norm_layer(data_format=data_format))
...
class EfficientNet(nn.Module):
        ...
        **kwargs: Any,
        ...
        for cnf in inverted_residual_setting:
            ...
            for _ in range(cnf.num_layers):
                ...
                stage.append(
                    block(block_cnf, sd_prob, norm_layer,
                          data_format=data_format)
                )
                stage_block_id += 1
        ...
        self.build(input_shape, data_format)
    ...
    def build(self, input_shape, data_format):
        shape = (
            (1, 3, input_shape, input_shape)
            if data_format == "channels_first"
            else (1, input_shape, input_shape, 3)
        )
        ones = tlx.ones(shape)
        _ = self(ones)
...
def _efficientnet(
    width_mult, depth_mult, dropout, input_shape, data_format="channels_first", **kwargs
):
...
def efficientnet(arch, data_format="channels_first", **kwargs):
    if arch == "efficientnet_b0":
        return _efficientnet(
            ...
            0.2,
            224,
            norm_layer=partial(nn.BatchNorm2d, epsilon=1e-5, momentum=0.1),
            data_format=data_format,
            **kwargs,
        )
