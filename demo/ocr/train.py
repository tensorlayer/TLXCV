...
def valid(model, test_dataset, limit=None):
    ...
    for index, (X_batch, y_batch) in enumerate(tqdm(test_dataset)):
        ...
        for predicted_id, text, input_id in zip(
            predicted_ids, y_batch[1], y_batch[0]["inputs"]
        ):
        ...
        for epoch in range(n_epoch):
            ...
            for X_batch, y_batch in train_dataset:
                ...
                with tf.GradientTape() as tape:
                    ...
                    _loss_ce = loss_fn(
                        _logits, input_ids=input_ids, attention_mask=attention_mask
                    )
...
if __name__ == "__main__":
    ...
    trainer = Trainer(
        network=model, loss_fn=model.loss_fn, optimizer=optimizer, metrics=None
    )
    trainer.train(
        n_epoch=1,
